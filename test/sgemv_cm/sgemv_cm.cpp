...
#pragma HLS DATAFLOW
  ...
  dyfc::blas::Matrix<float, Par, dyfc::blas::ColMajor> A_m(FLATTEN_MATRIX(A), dimM, dimN);
  dyfc::blas::Vector<float, Par> r_v(dimM);

  // Call a templated version of the blas function being tested
  dyfc::blas::mv<float, Par, dyfc::blas::ColMajor>(dimM, dimN, alpha, A_m, x_v, beta, y_v, r_v);
