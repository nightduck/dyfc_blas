...
#pragma HLS DATAFLOW
  ...
  dyfc::blas::Matrix<ComplexDouble, Par> A_m(FLATTEN_MATRIX(A), dimM, dimN);
  dyfc::blas::Vector<ComplexDouble, Par> r_v(dimM);

  // Call a templated version of the blas function being tested
  dyfc::blas::mv<ComplexDouble, Par, dyfc::blas::RowMajor>(dimM, dimN, alpha, A_m, x_v, beta, y_v,
                                                           r_v);
