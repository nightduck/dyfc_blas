...
void dgemv_rm(double alpha, double A[dimM][dimN], double x[dimN], double beta, double y[dimM], double r[dimM]) {
...
#pragma HLS DATAFLOW
  ...
  dyfc::blas::Matrix<double, Par> A_m(FLATTEN_MATRIX(A), dimM, dimN);
  dyfc::blas::Vector<double, Par> r_v(dimM);

  // Call a templated version of the blas function being tested
  dyfc::blas::mv<double, Par, dyfc::blas::RowMajor>(dimM, dimN, alpha, A_m, x_v, beta, y_v, r_v);
