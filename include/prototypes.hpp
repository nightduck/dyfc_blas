...
// This is effectively a TODO list of all the functions that need to be implemented

/*
 * ===========================================================================
 * Prototypes for level 1 BLAS functions. Vector ops.
 * Size and type information is removed in favor of parameterized types and sizes.
 * ===========================================================================
 */
...
void axpy(T alpha,
            Vector<T, Par, Length> &x,
            Vector<T, Par, Length> &y,
...
void scal(T alpha,
            Vector<T, Par, Length> &x,
            Vector<T, Par, Length> &result);
// TODO: Specific implementations for the standard: cscal, csscal, dscal, sscal, zdscal, zscal

/**
 * The BLAS copy operation is nonsensical on streams. If you want to slice or reshape a vector/matrix,
 * please use the provided class methods.
 */
template <typename T, unsigned int Par, unsigned int Length>
void copy(Vector<T, Par, Length> x,
            unsigned int incX,
            Vector<T, Par, Length> result) {
...
/**
 * The BLAS swap operation is not possible on streams, so this function will always assert false.
 */
template <typename T, unsigned int Par, unsigned int Length>
void swap(Vector<T, Par, Length> &x,
            Vector<T, Par, Length> &y)
{
 ...
 */
template <typename T, unsigned int Par, unsigned int Length>
void dot(Vector<T, Par, Length> &x,
            Vector<T, Par, Length> &y,
            T &result);
// Special implementation of dot where T is a complex number
void dotu();
// Special implementation of dotc where we take the conjugate of x before multiplying
void dotc();
// TODO: Specific implementations for the standard: cdotc, cdotu, ddot, dsdot, sdot, sdsdot, zdotc, zdotu
 ...
 */
template <typename T, unsigned int Par, unsigned int Length>
void nrm2(Vector<T, Par, Length> &x,
            T &result);
 ...
 */
template <typename T, unsigned int Par, unsigned int Length>
void asum(Vector<T, Par, Length> &x,
          T &result);
 ...
 */
template <typename T, unsigned int Par, unsigned int Length>
void amax(Vector<T, Par, Length> &x,
            unsigned int &result);
// TODO: Specific implementations for the standard: icamax, idamax, isamax, izamax

/**
 * Given the Cartesian coordinates (a, b) of a point, these routines return the parameters
 * c, s, r, and z associated with the Givens rotation. The parameters c and s define a unitary
 * matrix such that:
 *
 * [ c  s] [a] = [r]
 * [-s  c] [b] = [0]
 ...
 * @param[out] z A parameter that is equal to s if |a|>|b|, 1/c if c is not zero, and 1 otherwise.
 */
template <typename T>
...
// 1. Real a, b, c, s, r, z
// 2. Complex a, b, c, s, r, z  <- This is not in the standard, but it's a useful extension
 ...
 *
 * NOTE: May need to insist that s and r are both complex, deviating from the BLAS specification.
 *
 * xr = c * x + s * y
 * yr = c * y - conjg(s) * x
 * 
 ...
 */
template <typename T, unsigned int Par, unsigned int Length>
void rot(Vector<T, Par, Length> &x,
            Vector<T, Par, Length> &y,
            T &c,
            T &s,
            Vector<T, Par, Length> &xr,
            Vector<T, Par, Length> &yr);
template <typename T, unsigned int Par, unsigned int Length>
void rot(Vector<Complex<T>, Par, Length> &x,
            Vector<Complex<T>, Par, Length> &y,
            T &c,
            Complex<T> &s,
            Vector<Complex<T>, Par, Length> &xr,
            Vector<Complex<T>, Par, Length> &yr);
template <typename T, unsigned int Par, unsigned int Length>
void rot(Vector<Complex<T>, Par, Length> &x,
            Vector<Complex<T>, Par, Length> &y,
            T &c,
...
// TODO: Specific implementations for the standard: csrot, drot, srot, zrot, zdrot, crot


/**
 * Performs rotation of points in the plane. This is a modified version of rotg that's only
 ...
 * [ 0  ]     [ y1  sqrt(d2) ]
 *
 * Where H is a 2x2 matrix stored in row major order in the last 4 elements of params.
 * The first element of params is a flag element that is set as follows:
 *
 ...
 *                         [ 0.0  1.0 ]
 *
 * In the last three cases, the matrix entries of 1.0, -1.0, and 0.0 are assumed based on the
 * value of flag and are not required to be set in the param vector.
 *
 * @tparam T The type of the elements in the vector. Supports any type with defined arithmetic ops.
 * @tparam Par Number of elements retrieved in one read operation. Must be a power of 2.
 * @tparam Length The length of the vector.
...
// TODO: Add implementation for complex numbers, just to throw an unsupported warning
 ...
 */
template <typename T, unsigned int Par, unsigned int Length>
void rotm(Vector<T, Par, Length> &x,
          Vector<T, Par, Length> &y,
          Vector<T, Par, 5> &H,
          Vector<T, Par, Length> &xr,
          Vector<T, Par, Length> &yr);
 ...
 * r = alpha * A * x + beta * y
 * 
 * @tparam T The type of the elements in the matrix and vectors. Supports any type with defined arithmetic ops.
 * @tparam Par Number of elements retrieved in one read operation. Must be a power of 2.
 * @tparam Rows The number of rows in the matrix and the length of the resulting vector
 * @tparam Cols The number of columns in the matrix and the length of the input vector
 * 
...
void mv(T alpha,
        Matrix<T, Par, Rows, Cols, Order> &A,
        Vector<T, Par, Cols> &x,
        T beta,
        Vector<T, Par, Rows> &y,
        Vector<T, Par, Rows> &result);
// TODO: Subtemplates for gemv, hemv, symv, gbmv, hbmv, sbmv, hpmv, spmv
// TODO: Specific implementations for the standard: cgemv, dgemv, sgemv, zgemv, chemv, zhemv,
//       ssymv, dsymv, sgbmv, dgbmv, cgbmv, zgbmv, chbmv, zhbmv, ssbmv, dsbmv, chpmv, zhpmv,
//       sspmv, dspmv
// TODO: Original standard also takes options for if the matrix should be transposed (if general
//       or triangular), or if the diagonal is a unit
// TODO: The triangular cases are not specified in the standard with an addition operation, but
...
// TODO: Specific implementations for the standard: strmv, dtrmv, ctrmv, ztrmv, stbmv, dtbmv, ctbmv, ztbmv,
//       stpmv, dtpmv, ctpmv, ztpmv, ctpmv, ztpmv

/**
 * Computes a matrix-vector product for a triangular matrix and adds the result to a vector.
 * 
 * r = A * x
 * 
 * @tparam T The type of the elements in the matrix and vectors. Supports any type with defined arithmetic ops.
 ...
 * @param[out] result The output vector to write to.
 */
template<typename T, unsigned int Par, unsigned int Rows, Order=RowMajor, UpperLower UpLo=Upper>
void trmv(TriangularMatrix<T, Par, Rows, Order, UpLo> &A,
          Vector<T, Par, Rows> &x,
          Vector<T, Par, Rows> &result);
// TODO: Subtemplates for trmv, tbmv, tpmv
// TODO: Specific implementations for the standard: ctrmv, dtrmv, strmv, ztrmv, ctbmv, dtbmv, stbmv, ztbmv, ctpmv, dtpmv, stpmv, ztpmv
// TODO: Original standard also takes options for if the matrix should be transposed or if the diagonal is a unit

/**
 * Solves a system of linear equations whose coefficients are a triangular matrix.
 * 
 ...
 */
template<typename T, unsigned int Par, unsigned int Rows, Order=RowMajor, UpperLower UpLo=Upper>
void trsv(TriangularMatrix<T, Par, Rows, Order, UpLo> &A,
          Vector<T, Par, Rows> &x,
          Vector<T, Par, Rows> &result);
// TODO: Subtemplates for trsv, tbsv, tpsv
// TODO: Specific implementations for the standard: ctrsv, dtrsv, strsv, ztrsv, ctbsv, dtbsv, stbsv, ztbsv,
 ...
 * @tparam T The type of the elements in the matrix and vectors. Supports any type with defined arithmetic ops.
 * @tparam Par Number of elements retrieved in one read operation. Must be a power of 2.
 * @tparam Rows The number of rows in the matrix, and the length of the input vector x.
 * @tparam Cols The number of columns in the matrix, and the length of the input vector y.
 * 
...
void ger(T alpha,
         Vector<T, Par, Rows> &x,
         Vector<T, Par, Cols> &y,
         Matrix<T, Par, Rows, Cols> &A,
         Matrix<T, Par, Rows, Cols> &result);
// TODO: Subtemplates for syr, her, spr, hpr
// TODO: Specific implementations for the standard: cgeru, zgeru, dger, sger, ssyr, dsyr, cher, zher,
//       sspr, dspr, chpr, zhpr

/**
 * Performs a rank-1 update of a general matrix with complex numbers, using the conjugate transpose of y
 * 
 ...
 * @param[out] result The output matrix to write to.
 */
template<typename T, unsigned int Par, unsigned int Rows, unsigned int Cols, MajorOrder Order=RowMajorOrder>
void gerc(T alpha,
          Vector<Complex<T>, Par, Rows> &x,
          Vector<Complex<T>, Par, Cols> &y,
 ...
 * @tparam T The type of the elements in the matrix and vectors. Supports any type with defined arithmetic ops.
 * @tparam Par Number of elements retrieved in one read operation. Must be a power of 2.
 * @tparam Rows The number of rows in the matrix, and the length of the input vectors x and z.
 * @tparam Order The major order of the matrix. Can be either RowMajor or ColMajor.
 ...
 * @param[out] result The output matrix to write to.
 */
template<typename T, unsigned int Par, unsigned int Rows, MajorOrder Order=RowMajorOrder, UpperLower UpLo=Upper>
void syr2(T alpha,
          Vector<T, Par, Rows> &x,
          Vector<T, Par, Rows> &y,
 ...
 * @tparam K The number of columns in the matrix A and the number of rows in the matrix B.
 * @tparam N The number of columns in the matrix B and the output matrix result.
 * @tparam OrderA The major order of the matrix A. Can be either RowMajor or ColMajor.
 * @tparam OrderB The major order of the matrix B. Can be either RowMajor or ColMajor.
 * @tparam UpLo Whether the matrix is upper or lower triangular. This is ignored for general matrices
 * 
 * @param[in]  alpha The scalar to multiply the input matrix A by.
 * @param[in]  A The input matrix to multiply.
 ...
 * @param[out] result The output matrix to write to.
 */
template<typename T, unsigned int Par, unsigned int M, unsigned int K, unsigned int N, OrderA=RowMajor, OrderB=ColMajor, UpperLower UpLo=Upper>
void mm(T alpha,
        Matrix<T, Par, M, K, OrderA> &A,
        Matrix<T, Par, K, N, OrderB> &B,
        T beta,
        Matrix<T, Par, M, N, OrderA> &C,
        Matrix<T, Par, M, N, OrderA> &result);
// TODO: Subtemplates for gemm, hemm, symm, gemmtr
// TODO: Specific implementations for the standard: cgemm, dgemm, sgemm, zgemm, sgemmtr, dgemmtr, cgemmtr, zgemmtr,
//       chemm, zhemm, csymm, zsymm, ssymm, dsymm
// TODO: Original specificaiton takes options to pre-transpose A and B, as well as conjugate transpose
// TODO: Allow customization of the order of C and result (I think they should remain the same to each other)
...
// TODO: Specific implementations for the standard: cgemm, dgemm, sgemm, zgemm, strmm, dtrmm, ctrmm, ztrmm, chemm, zhemm, csymm, zsymm, ssymm, dsymm
 ...
 * @tparam Cols The number of columns in the matrix B and the output matrix result.
 * @tparam Order The major order of the matrix. Can be either RowMajor or ColMajor.
 * @tparam UpLo Whether the matrix is upper or lower triangular.
 * 
 * @param[in]  alpha The scalar to multiply the input matrix A by.
 ...
 * @param[out] result The output matrix to write to.
 */
template<typename T, unsigned int Par, unsigned int Rows, unsigned int Cols, Order=RowMajor, UpperLower UpLo=Upper>
void trsm(T alpha,
          TriangularMatrix<T, Par, Rows, Order, UpLo> &A,
          Matrix<T, Par, Rows, Cols> &B,
          Matrix<T, Par, Rows, Cols> &result);
// TODO: Specific implementations for the standard: ctrsm, dtrsm, strsm, ztrsm
// TODO: Original specificaiton takes options to pre-transpose A and B, as well as take their conjugate transpose
 ...
 * @tparam T The type of the elements in the matrix and vectors. Supports any type with defined arithmetic ops.
 * @tparam Par Number of elements retrieved in one read operation. Must be a power of 2.
 * @tparam Rows The number of rows in the matrices, A, C, and the output matrix result.
 * @tparam Cols The number of columns in the matrix A.
 * @tparam Order The major order of the matrix. Can be either RowMajor or ColMajor.
 * @tparam UpLo Whether the matrix C is upper or lower triangular.
 * 
 ...
 * @tparam T The type of the elements in the matrix and vectors. Supports any type with defined arithmetic ops.
 * @tparam Par Number of elements retrieved in one read operation. Must be a power of 2.
 * @tparam Rows The number of rows in the matrices, A, B, C, and the output matrix result.
 * @tparam Cols The number of columns in the matrices A and B.
 * @tparam Order The major order of the matrix. Can be either RowMajor or ColMajor.
 * @tparam UpLo Whether the matrix C is upper or lower triangular.
 * 
 * @param[in]  alpha The scalar to multiply the outer product of A*B^T and B*A^T by.
 * @param[in]  A The input matrix to multiply.
 * @param[in]  B The input matrix to multiply.
 * @param[in]  beta The scalar to multiply the input matrix C by.
 * @param[in]  C The input matrix to add to. Is square and symmetric.
...
void syr2k(T alpha,
           Matrix<T, Par, Rows, Cols, Order> &A,
           Matrix<T, Par, Rows, Cols, Order> &B,
           T beta,
           SymmetricMatrix<T, Par, Rows, Order, UpLo> &C,
           SymmetricMatrix<T, Par, Rows, Order, UpLo> &result);
// TODO: Specific implementations for the standard: csyr2k, dsyr2k, ssyr2k, zsyr2k
 ...
 * @tparam UpLo Whether the matrix C is upper or lower triangular.
 * 
 * @param[in]  alpha The scalar to multiply the outer product of A*conj(B)^T and B*conj(A)^T by.
 * @param[in]  A The input matrix to multiply.
 * @param[in]  B The input matrix to multiply.
 * @param[in]  beta The scalar to multiply the input matrix C by.
 * @param[in]  C The input matrix to add to. Is square and Hermitian.
...
void her2k(T alpha,
           Matrix<Complex<T>, Par, Rows, Cols, Order> &A,
           Matrix<Complex<T>, Par, Rows, Cols, Order> &B,
           T beta,
           HermitianMatrix<Complex<T>, Par, Rows, Order, UpLo> &C,
           HermitianMatrix<Complex<T>, Par, Rows, Order, UpLo> &result);
...
/**
 * Transposes a column-major matrix into a row-major matrix, or vice versa. Doesn't
 * move any values, just changes the type.
 */
template<typename T, unsigned int Par, unsigned int Rows, unsigned int Cols>
...
/**
 * Transposes a column-major matrix into a row-major matrix, or vice versa. This requires
 * rearranging the data in memory, so it's a more expensive operation.
 */
template<typename T, unsigned int Par, unsigned int Rows, unsigned int Cols>
